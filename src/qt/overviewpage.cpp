...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::SVALQO)
    {
    }
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("SVALQO")
    updateDisplayUnit();
}
...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeSVALQOAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeSVALQOAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeSVALQOAmount = strAnonymizeSVALQOAmount.remove(strAnonymizeSVALQOAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeSVALQOAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeSVALQOAmount * COIN) nMaxToAnonymize = nAnonymizeSVALQOAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeSVALQOAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeSVALQOAmount));
        strAnonymizeSVALQOAmount = strAnonymizeSVALQOAmount.remove(strAnonymizeSVALQOAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeSVALQOAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
